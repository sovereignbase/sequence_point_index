...
[[nodiscard]] inline std::uint32_t hash(std::uint32_t key_0, std::uint32_t key_1,
...
inline void resize_actors(std::uint32_t capacity) {
  ...
  for (std::uint32_t old = 0; old_actors && old < old_capacity; ++old) {
    if (!old_actors[old].values)
      continue;
    std::uint32_t slot =
        hash(old_actors[old].key_0, old_actors[old].key_1, old_actors[old].key_2) & next_mask;
...
[[nodiscard]] inline std::size_t find(std::uint32_t key_0, std::uint32_t key_1,
  ...
  while (actors[slot].values && (actors[slot].key_0 != key_0 || actors[slot].key_1 != key_1 ||
...
inline void erase_actor(std::size_t hole) {
  ...
  for (std::size_t slot = (hole + 1) & actor_mask; actors[slot].values;
    ...
    const std::size_t home =
        hash(actors[slot].key_0, actors[slot].key_1, actors[slot].key_2) & actor_mask;
...
[[nodiscard]] inline std::uint32_t read(std::uint32_t actor_0, std::uint32_t actor_1,
                                        std::uint32_t actor_2, std::uint32_t local_index) noexcept {
...
inline void write(std::uint32_t actor_0, std::uint32_t actor_1, std::uint32_t actor_2,
...
inline void remove(std::uint32_t actor_0, std::uint32_t actor_1, std::uint32_t actor_2,

...
[[nodiscard]] inline std::uint32_t hash(std::uint64_t key_hi, std::uint32_t key_lo) noexcept {
  return std::uint32_t(key_hi >> 32) ^ std::uint32_t(key_hi) ^ key_lo;
}
...
inline void resize_actors(std::uint32_t capacity) {
  ...
  for (std::uint32_t old = 0; old_actors && old < old_capacity; ++old) {
    if (!old_actors[old].values)
      continue;
    std::uint32_t slot = hash(old_actors[old].key_hi, old_actors[old].key_lo) & next_mask;
...
inline void resize_values(actor_slot& actor, actor_meta& meta, std::size_t capacity) {
...
[[nodiscard]] inline std::size_t find(std::uint64_t key_hi, std::uint32_t key_lo) noexcept {
  const std::size_t mask = actor_mask;
  std::size_t slot = hash(key_hi, key_lo) & mask;
  while (actors[slot].values && (actors[slot].key_hi != key_hi || actors[slot].key_lo != key_lo))
    slot = (slot + 1) & mask;
  return slot;
}
...
inline void erase_actor(std::size_t hole) {
  ...
  for (std::size_t slot = (hole + 1) & actor_mask; actors[slot].values;
    ...
    const std::size_t home = hash(actors[slot].key_hi, actors[slot].key_lo) & actor_mask;
...
[[nodiscard]] inline std::uint32_t read(std::uint64_t actor_hi, std::uint32_t actor_lo,
                                        std::uint32_t local_index) noexcept {
  detail::actor_slot* const actors = detail::actors;
  const std::size_t mask = detail::actor_mask;
  std::size_t slot = detail::hash(actor_hi, actor_lo) & mask;

  while (actors[slot].key_hi != actor_hi || actors[slot].key_lo != actor_lo)
    slot = (slot + 1) & mask;
  return actors[slot].values[local_index];
}

inline void write(std::uint64_t actor_hi, std::uint32_t actor_lo, std::uint32_t local_index,
                  std::uint32_t value) {
  const std::size_t index = local_index;

  if (!detail::actors)
    detail::resize_actors(16);
  std::size_t slot = detail::find(actor_hi, actor_lo);
  ...
  if (inserting) {
    ...
    if (detail::actor_count == capacity / 2) {
      ...
      slot = detail::find(actor_hi, actor_lo);
    }
    detail::actors[slot].key_hi = actor_hi;
    detail::actors[slot].key_lo = actor_lo;
  }
...
inline void remove(std::uint64_t actor_hi, std::uint32_t actor_lo, std::uint32_t local_index) {
  if (!detail::actors)
    return;

  const std::size_t index = local_index;
  const std::size_t slot = detail::find(actor_hi, actor_lo);
  detail::actor_slot& actor = detail::actors[slot];
  detail::actor_meta& meta = detail::metadata[slot];
